    ...
    try {
        ...
        if (user) {
            if (updateData.password && updateData.passwordOld) {
                ...
                user.name = updateData.firstName;
                user.phoneNumber = updateData.phoneNumber;
                if (updateData.address) {
                    const { error: addressError, value: validatedAddress } = addressSchema.validate(updateData.address);
                    if (addressError) {
                        return res.status(400).json({ error: addressError.details[0].message });
                    }
...
const forgotPassword = async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            return res.status(200).json({ message: "Check your email inbox to get a new password." });
        } else {
